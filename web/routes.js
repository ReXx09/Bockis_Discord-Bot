...
module.exports = function startWebServer({
  ...
  app.post('/api/git-recovery-run', dashboardAuth, (req, res) => {
    ...
    const baseScript = [
      ...
      '    echo "[Recovery] Unfertiger Merge erkannt -> git merge --abort"',
      '    git merge --abort >/dev/null 2>&1 || true',
      '    cleaned=true',
      '  fi',
      '  if [[ -d .git/rebase-apply || -d .git/rebase-merge ]]; then',
      '    echo "[Recovery] Unfertiger Rebase erkannt -> git rebase --abort"',
      '    git rebase --abort >/dev/null 2>&1 || true',
      '    cleaned=true',
      '  fi',
      '  if [[ -f .git/CHERRY_PICK_HEAD ]]; then',
      '    echo "[Recovery] Unfertiger Cherry-Pick erkannt -> git cherry-pick --abort"',
      '    git cherry-pick --abort >/dev/null 2>&1 || true',
      '    cleaned=true',
      '  fi',
      '  if [[ -f .git/REVERT_HEAD ]]; then',
      '    echo "[Recovery] Unfertiger Revert erkannt -> git revert --abort"',
      '    git revert --abort >/dev/null 2>&1 || true',
      '    cleaned=true',
      '  fi',
      '  if [[ "$cleaned" == "true" ]]; then',
      '    echo "[Recovery] Laufende Git-Operationen bereinigt"',
      '  fi',
      '  UNMERGED="$(git diff --name-only --diff-filter=U 2>/dev/null || true)"',
      '  if [[ -n "$UNMERGED" ]]; then',
      '    echo "[Recovery] Unaufgelöste Konflikte bestehen weiterhin:"',
      '    while IFS= read -r f; do [[ -n "$f" ]] && echo "[Recovery] Konflikt: $f"; done <<< "$UNMERGED"',
      '    return 1',
      '  fi',
      '}',
      'stash_step() {',
    ...
    const flowByStep = {
      stash: 'cleanup_step; stash_step',
      pull: 'cleanup_step; pull_step',
      pop: 'pop_step',
      all: 'cleanup_step; stash_step; pull_step; pop_step',
    };
  ...
  app.post('/api/config', dashboardAuth, (req, res) => {
    ...
    for (const key of ALLOWED_CFG) {
      ...
      if (key === 'DISCORD_ENABLED_COMMANDS') {
        const allowedCommands = new Set(['status', 'uptime', 'refresh', 'help', 'triggerinfo', 'coinflip', 'dice', 'eightball', 'cleanup', 'translate', 'ping', 'botinfo', 'serverstatus', 'ki', 'wetter', 'subscribe', 'remind', 'quote', 'poll', 'avatar', 'userinfo', 'testreply']);
